 ...
 * logoUrl 이 있으면 eyebrow 줄 앞에 매장 로고(높이 20px, 가로 최대 96px, 비율 유지)를 둔다 (docs/prd/11).
 */
export function AppHeader({ eyebrow, title, me, actions, width = "max-w-3xl", logoUrl }: { eyebrow: string; title: string; me: { id: string; nickname: string }; actions?: React.ReactNode; width?: string; logoUrl?: string | null }) {
  return (
      ...
      <div className={`mx-auto flex ${width} items-center justify-between gap-3 px-5 py-3`}>
        <div className="min-w-0">
          <p className="flex min-w-0 items-center gap-1.5 text-xs font-medium text-muted">
            {/* 로고 옆에 매장 이름이 같이 있으므로 이미지는 장식(alt="") */}
            {/* eslint-disable-next-line @next/next/no-img-element -- 인증 쿠키로 받는 API 이미지라 next/image 최적화 대상이 아니다 */}
            {logoUrl && <img src={logoUrl} alt="" className="h-5 w-auto max-w-24 shrink-0 object-contain" />}

...
## 2차 — 쿠키 세션 전환과 수정 요청·휴가·대타 (2026-09-25 오후)

액세스 토큰을 HttpOnly 쿠키로 옮기고 세션을 30일 슬라이딩으로 바꿨다 ([05](../prd/05-auth.md), 스킬 `timesheet-auth`). [08](../prd/08-correction-requests.md)·[09](../prd/09-leave-substitution.md) 를 추가했다.

| # | 무엇 | 로컬 | 운영 |
|---|---|---|---|
| S3 | `access_token`(`Path=/api`, `Max-Age=600`)·`refresh_token` 쿠키 속성, 본문에 토큰 없음 | ✅ | ✅ |
| S5·S6 | 쿠키 없음·변조·다른 aud·alg none → 401, **Bearer 헤더만 보내면 401** | ✅ | ✅ |
| X-1 | CSRF 헤더 없는 `POST /api/stores` → 403 `CSRF_TOKEN_INVALID`, 위조 Origin → 403 | ✅ | ✅ |
| X-2 | refresh 가 새 access 쿠키, 로그아웃이 두 쿠키 삭제 | ✅ | ✅ |
| S7~S9·S10~S13·S18·S19·S23 | 기존 시나리오 (쿠키판으로 다시) | ✅ | — (1차에서 확인, 로직 변경 없음) |
| S14 | 로그아웃 멱등 | ✅ | ✅ |
| S17 | `SESSION_MAX_AGE_SEC=5` 에서 6초 방치 → 401 | ✅ | — |
| S17b | 같은 서버에서 3초마다 refresh 3회 → 전부 200 (슬라이딩) | ✅ | — |
| C-1~C-7 | 수정·추가·삭제 요청과 승인·거절·취소, 중복 409, 남의 기록 404, 멤버 승인 403 | ✅ | ✅ |
| L·S·M | 휴가 겹침 409, 무급으로 바꿔 승인, 대타 수락 전 승인 409, 결근 아닌 날 계산, 직접 등록·삭제, 대기 수 | ✅ | ✅ |
| I-1~I-8 | 매장·초대 (쿠키판) | ✅ | ✅ |

- 로컬: e2e 15/15, S17·S17b 2/2, 급여 단위 테스트 21/21 (L-1~L-4, S-1 추가)
- 운영 테스트 데이터 삭제: 사용자 16, 매장 6. **테스트 계정이 아닌 사용자 2명·매장 1곳은 남겨 두었다**
- 브라우저 시나리오(새로고침 유지·탭 두 개·로그아웃 동기화)와 새 화면은 여전히 사람 확인이 필요하다
